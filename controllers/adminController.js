// RUTA: backend/controllers/adminController.js (v46.0 - ARQUITECTURA BSC-ONLY CONSOLIDADA)
...
async function _getBalancesForAddress(address, chain) {
    ...
    // --- MODIFICADO: Lógica simplificada para soportar solo BSC ---
    if (chain !== 'BSC') {
...
// --- FUNCIÓN RE-IMPLEMENTADA Y CORREGIDA (ANOMALÍA-020) ---
const getDashboardStats = asyncHandler(async (req, res) => {
    const totalUsersPromise = User.countDocuments();
    const totalDepositVolumePromise = User.aggregate([ { $group: { _id: null, total: { $sum: '$totalRecharge' } } } ]);
    ...
    const centralWalletBalancesPromise = (async () => {
        ...
        const walletsToScan = settings.centralWallets || [{ address: '0x_SU_DIRECCION_DE_TESORERIA_BSC', chain: 'BSC', name: 'Tesorería Principal BSC' }];
        const balanceResults = await Promise.all(walletsToScan.filter(w => w.chain === 'BSC').map(async (wallet) => {
            try { return { ...wallet, balances: await _getBalancesForAddress(wallet.address, wallet.chain), error: null }; }
            catch (error) { return { ...wallet, balances: { usdt: 0, bnb: 0 }, error: error.message }; }
        }));
        return balanceResults;
    })();
    const userGrowthDataPromise = User.aggregate([ { $match: { createdAt: { $gte: new Date(new Date().setDate(new Date().getDate() - 7)) } } }, { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } }, count: { $sum: 1 } } }, { $sort: { _id: 1 } } ]);
...
// --- INICIO DE ENDPOINTS RESTAURADOS Y REFACTORIZADOS PARA BSC-ONLY ---

const analyzeGasNeeds = asyncHandler(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 15;
    const chain = 'BSC'; // Hardcoded a BSC
    ...
    // Solo buscamos wallets que tengan USDT para ser candidatas.
    ...
    res.json({
        ...
        pagination: { currentPage: page, totalPages: Math.ceil(totalWalletsInChain / limit), totalWallets: totalWalletsInChain }
    });
});
...
const dispatchGas = asyncHandler(async (req, res) => {
    ...
    if (chain !== 'BSC') {
        res.status(400); throw new Error("Petición inválida. Solo se soporta BSC.");
    }
    if (!Array.isArray(targets) || targets.length === 0) {
        res.status(400); throw new Error("Petición inválida, se requiere un array de 'targets'.");
    }
...
// --- FIN DE ENDPOINTS RESTAURADOS ---

// El resto de funciones (CRUDs, 2FA, etc.) se mantienen igual que en la v45.7
...
const getAllTransactions = asyncHandler(async (req, res) => { const page = Number(req.query.page) || 1; const pageSize = 15; const typeFilter = req.query.type; const searchQuery = req.query.search; let pipeline = []; if (searchQuery) { pipeline.push({ $match: { $or: [ { username: { $regex: searchQuery, $options: 'i' } }, { telegramId: { $regex: searchQuery, $options: 'i' } } ] } }); } pipeline.push({ $unwind: '$transactions' }); if (typeFilter) { pipeline.push({ $match: { 'transactions.type': typeFilter } }); } pipeline.push({ $sort: { 'transactions.createdAt': -1 } }); const facetPipeline = [ ...pipeline, { $facet: { paginatedResults: [ { $skip: pageSize * (page - 1) }, { $limit: pageSize } ], totalCount: [ { $count: 'count' } ] } } ]; const results = await User.aggregate(facetPipeline); const paginatedItems = results[0].paginatedResults; const totalTransactions = results[0].totalCount[0] ? results[0].totalCount[0].count : 0; const transactions = paginatedItems.map(item => ({ ...item.transactions, user: { _id: item._id, username: item.username, telegramId: item.telegramId } })); res.json({ transactions, page, pages: Math.ceil(totalTransactions / pageSize), totalTransactions }); });
